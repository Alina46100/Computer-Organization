...
uint64_t* cache_sim_t::check_tag(uint64_t addr)
{
  ...
  for (size_t i = 0; i < ways; i++) {
    tag = &tags[(idx * ways) + i];
    if (*tag & VALID && ((*tag >> idx_shift) == (addr >> idx_shift))) {
      way = i;
      break;
    }
  }

  // If there was a cache miss, update the set queue
  if (way == 0 || !(*tag & VALID)) {
    set_queue[idx].push(way);
  }
...
uint64_t cache_sim_t::victimize(uint64_t addr)
{
  ...
  size_t idx = (addr >> idx_shift) & (sets - 1);
  size_t way = set_queue[idx].front();  // Get the first element (oldest line) in the queue
  set_queue[idx].pop();  // Remove the first element from the queue
  uint64_t victim = tags[idx * ways + way];
  tags[idx * ways + way] = (addr >> idx_shift) | VALID;
  set_queue[idx].push(way);  // Add the current way to the end of the queue
  return victim;
}
*/
uint64_t cache_sim_t::victimize(uint64_t addr)
{
  ...
  size_t idx = (addr >> idx_shift) & (sets - 1);
  size_t way = arr[idx];  // Get the first element (oldest line) in the queue
  uint64_t victim = tags[idx * ways + way];
  tags[idx * ways + way] = (addr >> idx_shift) | VALID;
