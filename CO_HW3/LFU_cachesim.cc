...
void cache_sim_t::init()
{
  ...
  miss_handler = NULL;
  access_counts.resize(sets * ways, 0);
...
{
  size_t idx = (addr >> idx_shift) & (sets - 1);
  size_t min_index = idx * ways;
  uint64_t min_count = access_counts[min_index];
  for (size_t i = 1; i < ways; i++)
  {
    size_t cur_index = idx * ways + i;
    uint64_t cur_count = access_counts[cur_index];
  ...
  uint64_t victim = tags[min_index];
  tags[min_index] = (addr >> idx_shift) | VALID;
  access_counts[min_index] = 1; // Reset the access count of the victim cache line
  return victim;
}
...
void cache_sim_t::access(uint64_t addr, size_t bytes, bool store)
{
  ...
  if (likely(hit_way != nullptr))
  {
    if (store)
      *hit_way |= DIRTY;
    access_counts[std::distance(tags, hit_way)]++; // Increment the access count for the hit cache line
    return;
  }
